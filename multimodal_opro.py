...
class MultiModalAPD:
    def __init__(self, num_prompts, starting_prompt, df_test, metaprompt_template_path, generation_model_name, generation_config, safety_settings, eval_model_name, eval_config, review_model_name, review_config, use_system_prompt):
        ...
        async with semaphore:
            try:
                ...
                if self.use_system_prompt:
                    ...
                    content = [prompt, image, row['question']]
                    eval_model = GenerativeModel(
                        self.eval_model_name,
                        generation_config=self.eval_config,
                        safety_settings=self.safety_settings
                ...
                return model_response, duration, token_usage, is_correct  # No extracted number needed
    ...
    async def evaluate_prompt(self, prompt, prompt_number):
        ...
        self.df_test[["pid", "question", "answer", "model_response", "review_response", "is_correct", "duration", "token_usage"]].to_csv(csv_file_path, index=False)
    ...
    async def main(self):
        ...
        for i in range(self.num_prompts+1):
            ...
            if i > 0 and i < self.num_prompts and i % 5 == 0:
            ...
            if i < self.num_prompts - 1:
                await aioconsole.aprint("Waiting for 70 seconds to avoid exceeding API quota...")
                async for _ in tqdm_asyncio(range(70), desc="Waiting", unit="s"):
...
if __name__ == "__main__":
    ...
    num_prompts = 100
    starting_prompt = """Examine the image provided. It contains a math problem. Solve this problem, ensuring you detail each step of your solution and explain the reasoning behind it."""
    
    # Load your DataFrame
    df_test = pd.read_pickle('apd-testdata.pkl')  # pickle ensures data type preservation
    ...
    use_system_prompt = False  # Set this flag to control prompt usage

    apd = MultiModalAPD(
        num_prompts, starting_prompt, df_test, 
        metaprompt_template_path, generation_model_name, generation_config, safety_settings, 
        eval_model_name, eval_config, review_model_name, review_config, use_system_prompt
    )
